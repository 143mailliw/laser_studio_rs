use eframe::egui;
use eframe::egui::text::{CCursorRange, CCursor};
...
pub fn get_position_from_range(range: CCursorRange, string: String) -> (u64, u64) {
    ...
    let mut remaining_chars: u64 = range.primary.index
        .try_into()
...
pub fn update_text_workspace(ctx: &egui::Context, app: &mut super::LaserStudioApp) {
    ...
    status_frame.inner_margin = egui::style::Margin {left: 7.0, right: 7.0, top: 5.0, bottom: 0.0};
    status_frame.fill = ctx.style().visuals.window_fill();
    status_frame.stroke = ctx.style().visuals.window_stroke();

    egui::TopBottomPanel::bottom("text_status").frame(status_frame).show(ctx, |ui| {
        let pos = get_position_from_range(app.text.cursor, app.project.text_data.content.clone());
    ...
    egui::CentralPanel::default().frame(frame).show(ctx, |ui| {
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                num_frame.inner_margin = egui::style::Margin {left: 2.0, right: 0.0, top: 2.0, bottom: 0.0};
                            ...
                            Some(rows) => {
                                ...
                                for row in rows {
                                    if last_had_new_line {
                                        ui.add(egui::Label::new(egui::RichText::new(cur_num.to_string()).size(14.0).text_style(egui::TextStyle::Monospace)));
                                    } else {
                                        ui.add(egui::Label::new(egui::RichText::new(" ").size(14.0).text_style(egui::TextStyle::Monospace)));
                                    }
                ...
                ui.vertical(|ui| {
                    ...
                    let response = egui::TextEdit::multiline(&mut app.project.text_data.content).code_editor().frame(false).desired_width(f32::INFINITY).show(ui);
                    match response.state.ccursor_range() {
                        Some(value) => { app.text.cursor = value; }
                        None => ()
                    }
                    app.text.rows = Some(response.galley.rows.clone());
                })
            }) 
        })
    });
